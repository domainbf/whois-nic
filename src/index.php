...
function cleanDomain($inputDomain = null)
{
    ...
    if (DEFINED_EXTENSION && strpos($domain, ".") === false) {
...
<body>
    ...
    <?php if ($whoisData || $rdapData): ?>
        ...
        <script>
            window.addEventListener("DOMContentLoaded", function() {
                ...
                if (typeof linkifyHtml !== 'undefined') {
                    const whoisElement = document.getElementById("raw-data-whois");
                    if (whoisElement) {
                        whoisElement.innerHTML = linkifyHtml(whoisElement.innerHTML, {
        ...
        <?php if ($fetchPrices): ?>
            <script>
                window.addEventListener("DOMContentLoaded", function() {
                    fetch("prices?domain=<?= urlencode($domain); ?>")
                        .then(response => response.json())
                        .then(data => {
                            const messagePrice = document.getElementById("message-price");
                            if (messagePrice && data.code === 0 && data.data) {
                                messagePrice.innerHTML = "";
                                data.data.forEach(item => {
                                    const div = document.createElement("div");
                                    div.className = "message-price-item";
                                    div.innerHTML = `
                                        <a href="${item.url}" rel="nofollow noopener noreferrer" target="_blank">${item.registrar}</a>: 
                                        ${item.price ? `${item.currency}${item.price}` : "不可用"}
                                    `;
                                    messagePrice.appendChild(div);
                                });
                            } else if (messagePrice) {
                                messagePrice.innerHTML = "<div>无法获取价格信息</div>";
                            }
                        })
                        .catch(() => {
                            const messagePrice = document.getElementById("message-price");
